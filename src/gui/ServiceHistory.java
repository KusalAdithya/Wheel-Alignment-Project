 ...
 * @author perer
 */
public class ServiceHistory extends javax.swing.JPanel {
    ...
    public void loadsHistory() {
        try {
            ...
            ResultSet rs = MySQL.search("SELECT * FROM `invoice` INNER JOIN `customer` ON `invoice`.`customer_id`=`customer`.`id` INNER JOIN `invoice_service` ON `invoice_service`.`invoice_id`=`invoice`.`id` INNER JOIN `service` ON `invoice_service`.`service_id`=`service`.`id` ORDER BY `invoice`.`date_time` DESC ");
    ...
    public void searchHistory() {
        try {
            ...
            String wherequery = "WHERE `invoice_service`.`service_id`>'0' ";
            ...
            ResultSet rs = MySQL.search("SELECT * FROM `invoice` INNER JOIN `customer` ON `invoice`.`customer_id`=`customer`.`id` INNER JOIN `invoice_service` ON `invoice_service`.`invoice_id`=`invoice`.`id` INNER JOIN `service` ON `invoice_service`.`service_id`=`service`.`id` " + wherequery + "  ORDER BY `invoice`.`date_time` DESC ");
            DefaultTableModel dtm = (DefaultTableModel) jTable1.getModel();
            dtm.setRowCount(0);
    ...
    private void initComponents() {
        ...
        setBorder(javax.swing.BorderFactory.createTitledBorder(new javax.swing.border.LineBorder(new java.awt.Color(204, 0, 0), 2, true), "Service History", javax.swing.border.TitledBorder.LEFT, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Rockwell", 0, 24), new java.awt.Color(204, 0, 0))); // NOI18N
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Invoice Id", "Car Number", "Service Id", "Service Name", "Date & Time"
            }
        ) {
